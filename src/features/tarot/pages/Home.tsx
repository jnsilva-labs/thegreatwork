...
import { CircleDot, ExternalLink, Infinity, Sparkles, Triangle } from '../icons';
      ...
      <div className="z-10 max-w-3xl w-full flex flex-col gap-12 animate-fade-in px-4">
          ...
          <div className="bg-void-900/80 backdrop-blur-sm border border-void-800 p-8 md:p-10 space-y-10 relative overflow-hidden">
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {INTENTIONS.map((option) => (
                  <button
                    ...
                    className={`px-4 py-1.5 text-[10px] uppercase tracking-widest transition-all border ${intention === option ? 'bg-alchemy-gold text-void-950 border-alchemy-gold font-bold' : 'bg-transparent text-slate-500 border-void-800 hover:border-alchemy-gold/50 hover:text-slate-300'}`}
                  >
                    {option}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-void-800/50">
              {[
                ...
                >
                  <spread.icon size={20} className="text-alchemy-gold/50 group-hover/btn:text-alchemy-gold transition-colors" />
                  <div>
                    <div className="font-headers text-lg text-slate-200 group-hover/btn:text-white transition-colors">{spread.name}</div>
                    <div className="text-[10px] uppercase tracking-widest text-slate-600 group-hover/btn:text-alchemy-gold/70">{spread.desc}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="w-full flex flex-col items-center gap-8">
          <div className="flex justify-center gap-12 text-[10px] uppercase tracking-[0.2em] text-slate-600 font-bold">
            <button onClick={() => onNavigate('journal')} className="flex items-center gap-2 hover:text-alchemy-gold transition-colors group">
              <span className="w-1 h-1 bg-current rounded-full group-hover:scale-150 transition-transform"></span> Journal
            </button>
            <button onClick={() => onNavigate('decks')} className="flex items-center gap-2 hover:text-alchemy-gold transition-colors group">
              <span className="w-1 h-1 bg-current rounded-full group-hover:scale-150 transition-transform"></span> Decks
            </button>
            <button onClick={() => onNavigate('settings')} className="flex items-center gap-2 hover:text-alchemy-gold transition-colors group">
              <span className="w-1 h-1 bg-current rounded-full group-hover:scale-150 transition-transform"></span> Settings
            </button>
          </div>

          <Link href="/" className="flex items-center gap-2 text-xs text-slate-700 hover:text-slate-400 transition-colors uppercase tracking-widest">
